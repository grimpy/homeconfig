...
class Socket(object):
    ...
    def receive(self):
        ...
        try:
            ...
            msg_magic, msg_length, msg_type = self.unpack_header()
            ...
            while len(self.buffer) < msg_size:
                self.buffer += self.socket.recv(msg_length)
            return self.unpack()
    ...
    def unpack(self):
        ...
        if data_size >= msg_size:
            payload = self.buffer[self.struct_header_size:msg_size].decode('utf-8')
    ...
    def unpack_header(self):
        ...
        """
        return struct.unpack(self.struct_header, self.buffer[:self.struct_header_size])
